...
let rpn_graph feature_map anchors_per_loc anchor_stride depth name =
  let dim = (Neuron.get_out_shape feature_map.neuron).(0) in
  assert (dim = (Neuron.get_out_shape feature_map.neuron).(1));
  ...
  let rpn_class_logits = reshape [|anchors_per_loc * dim * dim; 2|] x in
  ...
  let rpn_bbox = reshape [|anchors_per_loc * dim * dim; 4|] x in
